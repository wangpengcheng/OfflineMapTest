...
GeoTiledMapReply::GeoTiledMapReply(QNetworkAccessManager *networkManager, const QNetworkRequest &request, const QGeoTileSpec &spec, QObject *parent)
    : QGeoTiledMapReply(spec, parent), _reply(NULL), _request(request), _networkManager(networkManager)
{
...
//-----------------------------------------------------------------------------
void GeoTiledMapReply::replyDestroyed()
{
    _reply = 0;
}
...
//-----------------------------------------------------------------------------
//网络请求结束
void GeoTiledMapReply::networkReplyFinished()
{
    if (!_reply)
    {
        return;
...
//-----------------------------------------------------------------------------
//网络请求错误时的操作
void GeoTiledMapReply::networkReplyError(QNetworkReply::NetworkError error)
{
    if (!_reply)
    {
        return;
...
//-----------------------------------------------------------------------------
//缓冲错误时
void GeoTiledMapReply::cacheError(MapTask::TaskType type, QString /*errorString*/)
{
